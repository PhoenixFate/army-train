...
<body class="hideScroll">
		<form:form id="inputForm" modelAttribute="trainee" action="${ctx}/rzzk/trainee/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
				...
				<tr>
					...
					<td class="width-35">
						<input id="birthday" name="birthday" type="text" maxlength="20" class="laydate-icon form-control layer-date "
							value="<fmt:formatDate value="${trainee.birthday}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
					</td>
					<td class="width-15 active"><label class="pull-right"><font color="red">*</font>所属单位：</label></td>
					<td class="width-35">
						<sys:treeselect id="unit" name="unit.id" value="${trainee.unit.id}" labelName="unit.name" labelValue="${trainee.unit.name}"
							title="单位" url="/rzzk/armyTree/treeData?type=2" cssClass="form-control required" allowClear="true" notAllowSelectParent="true"/>
