...
import com.jeeplus.common.rzzk.SpringContextUtil;
...
public class EChartsRadarTag extends BodyTagSupport {
	...
	public int doEndTag() throws JspException {
		TrainPhysical trainPhysical=new TrainPhysical();
		TrainPhysicalService trainPhysicalService = (TrainPhysicalService) SpringContextUtil.getBean(TrainPhysicalService.class);
		List<TrainPhysical> list= trainPhysicalService.findList(trainPhysical);
		...
		if (polarType == 8) {
			...
			{
				polar.indicator(new Data().text(list.get(i).getName()).max(100));
			}
